...
describe("applyForJob", function () {
  test("works", async function () {
    ...
    await User.applyForJob({ username: "u1", jobId: testJob.rows[0].id });
  ...
  test("bad request if no such job", async function () {
    try {
      await User.applyForJob({ username: "u1", jobId: 1 });
      fail();
    } catch (err) {
      expect(err instanceof BadRequestError).toBeTruthy();
    }
  ...
  test("bad request if no such user", async function () {
    ...
    try {
      await User.applyForJob({ username: "u4", jobId: testJob.rows[0].id });
      fail();
    } catch (err) {
      expect(err instanceof BadRequestError).toBeTruthy();
    }
